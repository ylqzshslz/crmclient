	...
	<div>
		...
		<el-form :inline="true" :model="SaleChance" class="demo-form-inline el-main1">
			...
			<el-form-item>
				...
				<el-button type="primary" size="small" @click="addSaleChanceView()">新建</el-button>
			</el-form-item>
		</el-form>
		<el-table :data="results.data" style="width: 100%;margin-bottom: 20px;">
			...
			<el-table-column label="操作" width="120">
				<template slot-scope="scope">
					<el-button @click="dueSaleChanceView(scope.row)" type="text" size="small" v-show="$getSessionStorage('sysUser').userRoleId==2">
						<i class="el-icon-s-custom"></i>
					</el-button>
					<el-button @click="editSaleChanceView(scope.row)" type="text" size="small" v-show="scope.row.chanceCreateId == $getSessionStorage('sysUser').userId">
						<i class="el-icon-edit"></i>
					</el-button>
					<el-button @click="delSaleChanceEvent(scope.row)" type="text" size="small" v-show="scope.row.chanceCreateId == $getSessionStorage('sysUser').userId">
...
<script>
	export default {
		...
		methods: {
			...
			dueSaleChanceView(row) {       //参数是这一行，就是一个saleChance对象
				this.$router.push({
					path: 'dueSaleChance',
					query: {
						SaleChance: row    //把对象传给DueSaleChance
					}
				});
			},
